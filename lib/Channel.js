...
function Channel(info, client, opts) {
  ...
  */
  ...
  sshstream.once('CHANNEL_EOF:' + incoming.id, function() {
  ...
  }).once('CHANNEL_CLOSE:' + incoming.id, function() {
    ...
    channels.splice(channels.indexOf(incoming.id), 1);
    ...
    if (incoming.window === 0)
      windowAdjust(self);
  }).on('CHANNEL_WINDOW_ADJUST:' + incoming.id, function(amt) {
  ...
  }).on('CHANNEL_SUCCESS:' + incoming.id, function() {
    if (server) {
      sshstream._kalast = Date.now();
      sshstream._kacnt = 0;
    } else
      self._client._resetKA();
    if (self._callbacks.length)
      self._callbacks.shift()(false);
  }).on('CHANNEL_FAILURE:' + incoming.id, function() {
    if (server) {
      sshstream._kalast = Date.now();
      sshstream._kacnt = 0;
    } else
  ...
  }).on('CHANNEL_REQUEST:' + incoming.id, function(info) {
    if (!server) {
  ...
  if (server)
    ...
    sshstream.on('CHANNEL_EXTENDED_DATA:' + incoming.id,
      function(type, data) {
        incoming.window -= data.length;
