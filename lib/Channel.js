...
function Channel(info, conn) {
  ...
  */
  ...
  conn._parser.on('CHANNEL_SUCCESS:' + this.outgoing.id, function() {
    if (self._callbacks.length)
      self._callbacks.shift()(false);
  });

  conn._parser.on('CHANNEL_FAILURE:' + this.outgoing.id, function() {
    if (self._callbacks.length)
      self._callbacks.shift()(true);
  });

  conn._parser.on('CHANNEL_REQUEST:' + this.outgoing.id, function(info) {
    var err;
    if (info.request === 'exit-status' && info.code !== 0) {
      err = new Error('Exec returned exit code:' + info.code);
      err.exitCode = info.code;
    } else if (info.request === 'exit-signal') {
      err = new Error('Exec terminated by signal: SIG' + info.signal);
      err.signal = 'SIG' + info.signal;
      err.isCoreDump = info.coredump;
...
Channel.prototype._sendExec = function(cmd, cb, go) {
  ...
  */
  ...
  buf[14] = 0;
  buf.writeUInt32BE(cmdlen, 15, true);
  if (Buffer.isBuffer(cmd))
    cmd.copy(buf, 19);
  else
    buf.write(cmd, 19, cmdlen, 'utf8');
  this._callbacks.push(function(err) {
    if (err)
      cb(err);
    else
      cb(undefined, this._stream = new EventEmitter());
  });
  this._conn._send(buf);
