...
Channel.prototype._sendShell = function(cb, go) {
  ...
  */
  ...
  this._stream = new ChannelStream();
...
ChannelStream.prototype.write = function(data, encoding, extended) {
  ...
  if (Buffer.isBuffer(data)) {
    ...
    } else if (typeof extended !== 'number')
    ...
    if (this.paused) {
      ...
      if (extended)
        this._channel._sendData(data);
      else
        this._channel._sendData(data, extended);
    }
...
ChannelStream.prototype.resume = function() {
  this.paused = false;
  var i = 0, len = this._buffer.length;
