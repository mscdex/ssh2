...
Client.prototype.connect = function(cfg) {
  ...
  if (this.config.privateKey) {
    var privKey = parseKey(this.config.privateKey, cfg.passphrase);
    if (privKey instanceof Error)
      throw new Error('Cannot parse privateKey: ' + privKey.message);
    if (privKey.getPrivatePEM() === null)
  ...
  function tryNextAuth() {
    ...
    switch (curAuth) {
      ...
      break;
      case 'publickey':
        stream.authPK(self.config.username, self.config.privateKey);
        stream.once('USERAUTH_PK_OK', onUSERAUTH_PK_OK);
      break;
      case 'hostbased':
        function hostbasedCb(buf, cb) {
          var signature = self.config.privateKey.sign(buf);
