...
Client.prototype.connect = function(cfg) {
  ...
  this.config.promptForPassword = (cfg.promptForPassword === true);
  ...
  if (this.config.password !== undefined || this.config.promptForPassword !== undefined)
    auths.push('password');
  if (this.config.publicKey !== undefined
  ...
  function tryNextAuth() {
    ...
    switch (curAuth) {
      case 'password':
      	...
      	}else if(self.config.promptForPassword){
      	  self.emit('password',function(password){
      	  	 stream.authPassword(self.config.username,password);
      	  });
      	}
      break;
      case 'publickey':
      ...
      case 'hostbased':
        function hostbasedCb(buf, cb) {
          ...
          if (self.config.privateKey.type === 'dss' && signature.length > 40) {
            // this is a quick and dirty way to get from DER encoded r and s
            // that OpenSSL gives us, to just the bare values back to back (40
            // bytes total) like OpenSSH (and possibly others) are expecting
            var newsig = new Buffer(40);
            var rlen = signature[3];
            var rstart = 4;
            var sstart = 4 + 1 + rlen + 1;
  ...
  function onUSERAUTH_PK_OK(keyAlgo, key) {
    if (curAuth === 'agent') {
      ...
      stream.authPK(self.config.username,
                    ...
                    function(buf, cb) {
        var algo;
        switch (self.config.privateKey.type) {
          case 'rsa':
            algo = 'RSA-SHA1';
            break;
          case 'dss':
            ...
            break;
          case 'ec':
            switch (self.config.privateKey.curve) {
              case 'nistp256':
                algo = 'sha256';
                break;
              case 'nistp384':
