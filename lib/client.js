...
Client.prototype.connect = function(cfg) {
  ...
  function sendKA() {
    ...
    if (stream.writable) {
      ...
      callbacks[callbacks.length] = resetKA;
      stream.ping();
    } else
      clearInterval(katimer);
  }
...
Client.prototype.forwardIn = function(bindAddr, bindPort, cb) {
  ...
  if (wantReply) {
    this._callbacks.push(function(had_err, data) {
      if (had_err)
        return cb(new Error('Unable to bind to ' + bindAddr + ':' + bindPort));
...
Client.prototype.unforwardIn = function(bindAddr, bindPort, cb) {
  ...
  if (wantReply) {
    this._callbacks.push(function(had_err) {
      if (had_err)
        return cb(new Error('Unable to unbind from ' + bindAddr + ':' + bindPort));

      var forwards = self._forwarding;
      forwards.splice(forwards.indexOf(bindAddr + ':' + bindPort), 1);
...
Client.prototype.openssh_forwardInStreamLocal = function(socketPath, cb) {
  ...
  if (!this.config.strictVendor
    ...
    if (wantReply) {
      this._callbacks.push(function(had_err) {
        if (had_err)
          return cb(new Error('Unable to bind to ' + socketPath));
...
Client.prototype.openssh_unforwardInStreamLocal = function(socketPath, cb) {
  ...
  if (!this.config.strictVendor
    ...
    if (wantReply) {
      this._callbacks.push(function(had_err) {
        if (had_err)
          return cb(new Error('Unable to unbind on ' + socketPath));
...
function openChannel(self, type, opts, cb) {
  ...
  if (type === 'session')
    ...
    ret = self._sshstream.openssh_directStreamLocal(localChan,
                                                    initWindow,
                                                    maxPacket,
                                                    opts);
  }
...
function reqAgentFwd(chan, cb) {
  ...
  return chan._client._sshstream.openssh_agentForward(chan.outgoing.id, wantReply);
}
