...
Client.prototype.exec = function(cmd, opts, cb) {
  ...
  return openChannel(this, 'session', extraOpts, function(err, chan) {
    ...
    if (self.config.allowAgentFwd === true && !self._agentFwded) {
      todo.push(function() {
        reqAgentFwd(chan, function(err) {
          if (err)
            return reqCb(err);
...
Client.prototype.forwardIn = function(bindAddr, bindPort, cb) {
  ...
  this._sshstream.tcpipForward(bindAddr, bindPort, true);
};
...
Client.prototype.unforwardIn = function(bindAddr, bindPort, cb) {
  ...
  this._sshstream.cancelTcpipForward(bindAddr, bindPort, true);
};
...
function reqX11(chan, screen, cb) {
  ...
  return chan._client._sshstream.x11Forward(chan.outgoing.id, cfg, true);
}
...
function reqAgentFwd(chan, cb) {
  ...
  return chan._client._sshstream.agentForward(chan.outgoing.id, true);
}
