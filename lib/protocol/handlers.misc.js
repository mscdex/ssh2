          ...
          if (hasSig) {
            ...
            if (signature !== undefined) {
              if (signature.length > (4 + keyAlgo.length + 4)
                ...
                if (keyAlgo.indexOf('sk-') === 0) {
                  // FIDO/U2F (SK) signatures: leave the sigblob
                  // (string raw_sig | byte flags | uint32 counter) intact
                  // for the SK key's verify() to parse.
                  signature = bufferSlice(signature, 4 + keyAlgo.length);
                } else {
