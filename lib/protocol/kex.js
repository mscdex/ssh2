...
const createKeyExchange = (() => {
  ...
  class MLKEMx25519Exchange extends KeyExchange {
    ...
    async generateKeys() {
      ...
      if (!this._mlkemKeys) {
        const mlkemKeyPair = await crypto.subtle.generateKey(
          { name: 'ml-kem-768' },
          true,
          ['encapsulateKey', 'decapsulateKey']
        );

        const spkiExport = await crypto.subtle.exportKey(
          'spki',
          mlkemKeyPair.publicKey
        );

        const mlKemPublicKeySize = 1184;

        this._mlkemKeys = {
          // HACK: avoids parsing header.
          // Similar to what happens on Curve25519Exchange
          publicKey: Buffer.from(spkiExport).slice(-mlKemPublicKeySize),
    ...
    async getPublicKey() {
      ...
      const x25519Pub = this._x25519Keys.publicKey
        .export({ type: 'spki', format: 'der' })
        .slice(-32); // HACK: avoids parsing DER/BER header

      return Buffer.concat([this._mlkemKeys.publicKey, x25519Pub]);
    }
    ...
    async start() {
      ...
      // NOTE: Check 2.2 on the following draft
      // https://datatracker.ietf.org/doc/html/draft-ietf-sshm-mlkem-hybrid-kex-03.html
      const SSH_MSG_KEX_HYBRID_INIT = MESSAGE.KEXDH_INIT;

      packet[p] = SSH_MSG_KEX_HYBRID_INIT;
    ...
    if (typeof kexType === 'string') {
      ...
      switch (kexType) {
        case 'mlkem768x25519-sha256':
          if (!mlkemSupported || !curve25519Supported)
            break;
