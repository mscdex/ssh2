...
class Protocol {
  ...
  disconnect(reason, message = '', lang = '') {
    ...
    writeUInt32BE(packet, msgLen, p += 4);
    if (msgLen > 0){
      packet.utf8Write(message, p += 4, msgLen);
    } else {
      // This is because the SSH protocol expects a 4-byte length field for the message,
      // followed by the message data (which may be zero bytes).
      // Advancing the pointer by 4 ensures the next field (the language tag)
      // is written at the correct offset, maintaining the correct packet structure.
      p += 4;
    }

    writeUInt32BE(packet, langLen, p += msgLen);
    if (langLen > 0) packet.utf8Write(lang, p += 4, langLen);
