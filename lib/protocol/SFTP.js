...
console.log(`Writing file to sftp with length ${buf.length} offset ${off} length ${len}  position ${position}`);
    if (overflow)
      len = maxDataLen;
  ...
  fastGet(remotePath, localPath, opts, cb) {
    ...
    const fileSystemAdapter = opts.customFs ? opts.customFs : fs;
    fastXfer(this, fileSystemAdapter, remotePath, localPath, opts, cb);
  }
  fastPut(localPath, remotePath, opts, cb) {
    ...
    const fileSystemAdapter = opts.customFs ? opts.customFs : fs;
    fastXfer(fileSystemAdapter, this, localPath, remotePath, opts, cb);
  }
...
function fastXfer(src, dst, srcPath, dstPath, opts, cb) {
  ...
  if (typeof opts === 'function') {
    ...
    if (typeof opts.offset === 'number'
      && opts.offset >= 0
      && !isNaN(opts.offset)) {
      // This is not working as expected so I'll disable it altogether for now
      // offset = opts.offset;
      offset = 0;
    }
  ...
  src.open(srcPath, 'r', (err, sourceHandle) => {
    ...
    function tryStat(err, attrs) {
      ...
      fsize = attrs.size;
      fileLength = fileLength ? fileLength : fsize;
      if (offset + fileLength > fsize) {
        return onerror(new Error("Offset and length exceed file size"));
      }

      console.log('filesize', fsize, fileLength, offset);
      dst.open(dstPath, 'w', (err, destHandle) => {
        ...
        if (fsize <= 0 || fileLength <= 0)
          return onerror();

        // Use less memory where possible
        while (bufsize > fileLength) {
          if (concurrency === 1) {
            bufsize = fileLength;
            break;
          }
        ...
        function onread(err, nb, data, dstpos, datapos, origChunkLen) {
          ...
          function writeCb(err) {
            ...
            total += nb;
            onstep && onstep(total, nb, fileLength);

            if (nb < origChunkLen) {
              console.log(`Reading the rest of the chunk for the original position ${dstpos}, missing ${origChunkLen-nb} bytes. New position will be ${dstpos + nb}`);
            ...
            const chunk =
              (pdst + chunkSize > fileLength ? fileLength - pdst : chunkSize);
        ...
        function startReads() {
          let reads = 0;
          let psrc = 0;
          while (pdst < fileLength && reads < concurrency) {
            const chunk =
              (pdst + chunkSize > fileLength ? fileLength - pdst : chunkSize);
            singleRead(psrc, pdst + offset, chunk);
            psrc += chunk;
            pdst += chunk;
            ++reads;
          }
