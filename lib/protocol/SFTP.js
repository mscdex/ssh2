...
class SFTP extends EventEmitter {
  ...
  fastPutSled(sourceInterface, localPath, remotePath, opts, cb) {
    if (this.server)
      throw new Error('Client-only method called in server mode');
    fastXfer(sourceInterface, this, localPath, remotePath, opts, cb);
  }
  ...
  src.open(srcPath, 'r', (err, sourceHandle) => {
    ...
    function tryStat(err, attrs) {
      ...
      dst.open(dstPath, 'w', (err, destHandle) => {
        ...
        console.log('Buffer size set', bufsize);
        ...
        function onread(err, nb, data, dstpos, datapos, origChunkLen) {
          ...
          // console.log('Writing', dstHandle, readbuf, datapos, nb, dstpos);
          ...
          function writeCb(err) {
            ...
            console.log('Write cb', total, nb, fsize, origChunkLen);
            total += nb;
