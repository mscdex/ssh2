    ...
    function tryStat(err, attrs) {
      ...
      dst.open(dstPath, 'w', (err, destHandle) => {
        ...
        function onread(err, nb, data, dstpos, datapos, origChunkLen) {
          ...
          console.log(`After getting chunk for position ${dstpos}, the chunk has ${nb} bytes out of ${origChunkLen}`);
          ...
          function writeCb(err) {
            ...
            const chunk =
              (pdst + chunkSize > length ? length - pdst : chunkSize);
        ...
        function startReads() {
          let reads = 0;
          let psrc = 0;
          while (pdst < length && reads < concurrency) {
            const chunk =
              (pdst + chunkSize > length ? length - pdst : chunkSize);
