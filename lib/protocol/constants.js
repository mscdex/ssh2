...
const mlkemSupported = (() => {
  try {
    if (!crypto.generateKeyPairSync || typeof crypto.generateKeyPairSync !== 'function')
      return false;

    const algoName = 'ml-kem-768';
    const test = crypto.generateKeyPairSync(algoName);
    return !!test.publicKey && !!test.privateKey;
  } catch {
    return false;
  }
})();
