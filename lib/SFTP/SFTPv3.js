  ...
  src.stat(srcPath, function(err, attrs) {
    ...
    src.open(srcPath, 'r', function(err, sourcefd) {
      ...
      dst.open(dstPath, 'w', function(err, destfd) {
        ...
        function onread(err, nb, data, datapos, dstpos) {
          if (err) return onerror(err);

          dst.write(destfd, data, datapos, nb, dstpos, function(err) {
            if (err) return onerror(err);
        ...
        function read() {
          while (pdst < fsize && reads < concurrency) {
            ...
            // normalize this since fs.read passes different arguments than
            // sftp.read to the callback
            (function(bufpos, dstpos) {
              src.read(sourcefd, readbuf, bufpos, chunk, dstpos,
                function(err, nb, data) {
                  onread(err, nb, data, src === fs ? bufpos : 0, dstpos);
                }
              );
            })(psrc, pdst);
            psrc += chunk;
