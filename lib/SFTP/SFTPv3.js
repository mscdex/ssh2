...
SFTP.prototype.fastGet = function(remotePath, localPath, opts, cb) {
  ...
  var fsize, chunk, pr = 0, pl = 0, reads = 0, total = 0, lfd, rfd, self = this,
  ...
  function onerror(err) {
    ...
    if (lfd) {
      ++left;
      fs.close(lfd, cbfinal);
    }
    if (rfd) {
      ++left;
      self.close(rfd, cbfinal);
    }
  }
  ...
  self.stat(remotePath, function(err, attrs) {
    ...
    self.open(remotePath, 'r', function(err, remotefd) {
      ...
      fs.open(localPath, 'w', function(err, localfd) {
        ...
        function onread(err, nb, data, pos) {
          if (err) return onerror(err);

          fs.write(localfd, data, 0, nb, pos, function(err) {
            if (err) return onerror(err);

            if (--reads === 0) {
              if (total === fsize) {
                fs.close(localfd, function(err) {
                  lfd = undefined;
                  if (err) return onerror(err);
                  self.close(remotefd, function(err) {
                    rfd = undefined;
                    if (err) return onerror(err);
                    cb();
                  });
        ...
        function read() {
          while (pr < fsize && reads < concurrency) {
            chunk = (pr + chunkSize > fsize ? fsize - pr : chunkSize);
            self.read(remotefd, readbuf, pl, chunk, pr, onread);
            pl += chunk;
            pr += chunk;
            ++reads;
          }
...
function attrsToBytes(attrs) {
  ...
  if (typeof attrs.permissions === 'number'
    ...
    var mode = modeNum(attrs.permissions);
