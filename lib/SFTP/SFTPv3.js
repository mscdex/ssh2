...
SFTP.prototype.readFile = function(path, options, callback_) {
  ...
  this.open(path, flag, 438 /*=0666*/, function(er, fd_) {
    ...
    self.fstat(fd, function(er, st) {
      if (er) {
        return self.close(fd, function() {
  ...
  function read() {
    if (size === 0) {
      ...
      self.read(fd, buffer, 0, 8192, bytesRead, afterRead);
    } else
      self.read(fd, buffer, pos, size - pos, bytesRead, afterRead);
  }
    ...
    if (openErr)
      ...
      if (position === null) {
        self.fstat(fd, function(er, st) {
          ...
          self._writeAll(fd, buffer, 0, buffer.length, st.size, callback);
        });
        return;
      }
      self._writeAll(fd, buffer, 0, buffer.length, position, callback);
    }
  });
};
...
SFTP.prototype._writeAll = function(fd, buffer, offset, length, position, callback_) {
  var callback = (typeof callback_ === 'function' ? callback_ : undefined);
  var self = this;
  ...
  this.write(fd, buffer, offset, length, position, function(writeErr, written) {
    ...
    if (written === length)
      ...
      length -= written;
      position += written;
      self._writeAll(fd, buffer, offset, length, position, callback);
    }
  });
};
...
ReadStream.prototype.open = function() {
  var self = this;
  this.sftp.open(this.path, this.flags, this.mode, function(er, fd) {
    if (er) {
      if (self.autoClose)
        self.destroy();
...
ReadStream.prototype._read = function(n) {
  ...
  // the actual read.
  var self = this;
  this.sftp.read(this.fd, pool, pool.used, toRead, this.pos, onread);
...
ReadStream.prototype.close = function(cb) {
  ...
  if (cb)
    this.once('close', cb);
  if (this.closed || !Buffer.isBuffer(this.fd)) {
    if (!Buffer.isBuffer(this.fd)) {
  ...
  function close(fd) {
    self.sftp.close(fd || self.fd, function(er) {
      if (er)
        self.emit('error', er);
      else
        self.emit('close');
...
WriteStream.prototype.open = function() {
  ...
  this.sftp.open(this.path, this.flags, this.mode, function(er, fd) {
    ...
    if (self.flags[0] === 'a') {
      self.sftp.fstat(fd, function(err, st) {
        if (err) {
          self.destroy();
          self.emit('error', err);
          return;
...
WriteStream.prototype._write = function(data, encoding, cb) {
  ...
  var self = this;
  this.sftp.write(this.fd, data, 0, data.length, this.pos, function(er, bytes) {
    if (er) {
      self.destroy();
      return cb(er);
    }
