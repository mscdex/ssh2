...
function attrsToBytes(attrs) {
  ...
  if (typeof attrs.permissions === 'number') {
    ...
    attrBytes += 4;
    ret.push([(attrs.permissions >> 24) & 0xFF,
  ...
  if (typeof attrs.atime === 'number' && typeof attrs.mtime === 'number') {
    ...
    attrBytes += 8;
    ret.push([(attrs.atime >> 24) & 0xFF, (attrs.atime >> 16) & 0xFF,
              (attrs.atime >> 8) & 0xFF, attrs.atime & 0xFF]);
    ret.push([(attrs.mtime >> 24) & 0xFF, (attrs.mtime >> 16) & 0xFF,
...
function ReadStream(sftp, path, opts) {
  ...
  if (opts.start !== undefined) {
    ...
    this.pos = this.start = opts.start;
  }
  ...
  sftp._stream._conn._sock.once('end', function() {
    self.readable = false;
  });
  sftp._stream._conn._sock.once('close', function() {
    self.readable = false;
  });
}
...
ReadStream.prototype._read = function() {
  ...
  // one.
  var thisPool = pool,
      toRead = Math.min(pool.length - pool.used, ~~this.bufferSize),
      start = pool.used;
...
function WriteStream(sftp, path, opts) {
  ...
  this.busy = false;
  this._queue = [[this._open, this.path, this.flags, this.mode, undefined]];
  this._open = self._sftp.open;
  this.flush();
}
