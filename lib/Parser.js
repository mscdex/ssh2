...
Parser.prototype.execute = function(b, start, end) {
  ...
  var i = start, buffer, skipDecrypt = false;
  while (true) {
    ...
    switch (this._state) {
      ...
      case STATE_PACKETDATA:
        if (this._decrypt && !skipDecrypt)
          ...
          skipDecrypt = false;
        var buf, padStart = this._pktLen - this._padLen - 1;
        if (this._pktExtra) {
        ...
        switch (payload[0]) {
          ...
          case MESSAGE.USERAUTH_BANNER:
            /*
              ...
              string    language tag
            */
            var message = readString(payload, 1, 'utf8'),
          ...
          case 60: // user auth context-specific messages
            if (this._authMethod === 'password') {
              ...
              */
              var name = readString(payload, 1, 'utf8'),
            ...
            } else if (this._authMethod === 'publickey') {
              /*
          ...
          case MESSAGE.CHANNEL_OPEN:
            ...
            */
            var channel = {
              type: readString(payload, 1, 'ascii'),
              sender: payload.readUInt32BE(payload._pos, true),
              initWndSize: payload.readUInt32BE(payload._pos += 4, true),
              maxPktSize: payload.readUInt32BE(payload._pos += 4, true),
              data: {}
            };
            if (channel.type === 'forwarded-tcpip') {
              /*
              ...
              */
              channel.data.destIP = readString(payload, payload._pos += 4, 'ascii');
              channel.data.destPort = payload.readUInt32BE(payload._pos, true);
              channel.data.srcIP = readString(payload, payload._pos += 4, 'ascii');
              channel.data.srcPort = payload.readUInt32BE(payload._pos, true);
            }
