...
var EXP_BYTES_CRLF = bytes('\r\n'),
    ...
    EXP_BYTES_DASH = bytes('-');

var EXP_ACTION_NONE = 0,
    EXP_ACTION_BUFFER = 1,
    EXP_ACTION_EMIT = 2;

var EXP_TYPE_MATCH = 0,
    EXP_TYPE_BYTES = 1;
...
/*if (this._expectLen < 20)
  console.log('EXP_TYPE_BYTES: buf: ' + inspect(this._expectBuf));
else
  console.log('EXP_TYPE_BYTES (' + this._expectPtr + '/' + this._expectLen + ')');*/
        if (this._expectPtr === this._expectLen) {
//console.log('EXP_TYPE_BYTES met length');
...
//console.log('b[i]:', require('util').inspect(String.fromCharCode(b[i])) + ', i:', i + ', start:', start + ', ptr:', this._expectPtr + ', len:', this._expectLen + ', buf:', require('util').inspect(this._expectBuf.toString()));
