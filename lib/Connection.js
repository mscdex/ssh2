...
Connection.prototype.connect = function(opts) {
  ...
  if (this._privateKey) {
    ...
    if (!keyInfo)
      throw new Error('Error while parsing given privateKey');
    if (!keyInfo.private)
      throw new Error('Given privateKey does not contain a private key');
  ...
  if (this._publicKey && !this._privateKey.public) {
    ...
    if (!keyInfo)
      throw new Error('Error while parsing given publicKey');
    if (!keyInfo.public)
...
Connection.prototype.exec = function(cmd, env, cb) {
  ...
  this._openChan(function(err, chan) {
    if (err)
      return cb(err);
