...
console.log('packet type: ' + type + ' (' + typeid + ')');
    self._debug&&self._debug('packet', type, typeid, data);
  });
  this._parser.on(MESSAGE.DEBUG, function(msg, lang) {
    self._debug&&self._debug('debug', msg, lang);
  });
  this._parser.on(MESSAGE.KEXINIT, function(init) {
  ...
  this._parser.on(MESSAGE.KEXDH_REPLY, function(info) {
    ...
    info.secret = self._kex.computeSecret(info.pubkey, 'binary', 'binary');
    // SHA1 for both currently supported DH kex methods
    var hash = crypto.createHash('sha1');
    hash.update(SSH_IDENT, 'utf8');
    hash.update(self._server_ident_raw, 'utf8');
    hash.update(self._kexinit);
    hash.update(self._parser._kexinit);
    hash.update(info.hostkey);
    hash.update(self._pubkey);
    hash.update(info.pubkey);
    hash.update(info.secret, 'binary');
    self._exchange_hash = hash.digest('binary');
    if (self._sessionid === undefined)
      self._sessionid = self._exchange_hash;
    self._kexreply = info;
    self._send(new Buffer([MESSAGE.NEWKEYS]));
  });
  this._parser.on(MESSAGE.NEWKEYS, function() {
    var iv, key, blocklen = 8, keylen = 0,
        secret = self._kexreply.secret,
        iv = crypto.createHash('sha1').update(secret + self._exchange_hash + 'A'
                                              + self._sessionid, 'binary')
                                      .digest('binary');
    ...
    while (blocklen > Buffer.byteLength(iv)) {
      iv += crypto.createHash('sha1').update(secret + self._exchange_hash
                                             + iv, 'binary')
                                     .digest('binary');
    }
    iv = new Buffer(iv, 'binary').slice(0, blocklen);
    switch (self._encryptType) {
      case 'aes256-cbc':
      case 'aes256-ctr':
      case 'arcfour256':
    ...
    key = crypto.createHash('sha1').update(secret + self._exchange_hash + 'C'
                                           + self._sessionid, 'binary')
                                   .digest('binary');
      while (keylen > Buffer.byteLength(key)) {
        key += crypto.createHash('sha1').update(secret + self._exchange_hash
                                                + key, 'binary')
                                        .digest('binary');
      }
    key = new Buffer(key, 'binary').slice(0, keylen);
    this._encrypt = crypto.createCipheriv(SSH_TO_OPENSSL[self._encryptType],
                                          key, iv);
    this._encrypt.setAutoPadding(false);

    // and now for decrypting ...

    blocklen = 8;
    keylen = 0;
    iv = crypto.createHash('sha1').update(secret + self._exchange_hash + 'B'
                                          + self._sessionid, 'binary')
                                  .digest('binary');
    ...
    key = crypto.createHash('sha1').update(secret + self._exchange_hash + 'D'
                                           + self._sessionid, 'binary')
                                   .digest('binary');
    while (keylen > Buffer.byteLength(key)) {
      key += crypto.createHash('sha1').update(secret + self._exchange_hash
    ...
    key = new Buffer(key, 'binary').slice(0, keylen);
    self._parser._decrypt = crypto.createCipheriv(SSH_TO_OPENSSL[self._parser
                                                                 ._decryptType],
                                                  key, iv);
    ...
    key = crypto.createHash('sha1').update(secret + self._exchange_hash + 'E'
                                           + self._sessionid, 'binary')
                                   .digest('binary');
    while (createKeyLen > Buffer.byteLength(key)) {
      key += crypto.createHash('sha1').update(secret + self._exchange_hash
                                              + key, 'binary')
                                      .digest('binary');
    }
    self._hmacKey = key;
    key = crypto.createHash('sha1').update(secret + self._exchange_hash + 'F'
                                           + self._sessionid, 'binary')
                                   .digest('binary');
    while (checkKeyLen > Buffer.byteLength(key)) {
      key += crypto.createHash('sha1').update(secret + self._exchange_hash
                                              + key, 'binary')
                                      .digest('binary');
    }
    self._parser._hmacKey = key;
