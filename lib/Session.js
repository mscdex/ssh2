...
var Session = function(opts) {
  ...
  this._parser.on(MESSAGE.SERVICE_ACCEPT, function(svc) {
    var buf, p = 0;
    if (svc === 'ssh-userauth') {
      if (self._password)
        self._authPwd();
...
Session.prototype._authPwd = function() {
  /*
  ...
  */
  var userLen = Buffer.byteLength(self._username),
      passLen = Buffer.byteLength(self._password);
  buf = new Buffer(1
                   + 4 + userLen
                   + 4 + 14 // "ssh-connection"
                   + 4 + 8 // "password"
  ...
  buf.write(self._username, p += 4, userLen, 'ascii');
  ...
  buf.write(self._password, p += 4, passLen, 'utf8');
  self._send(buf);
};
