...
function openChannel(self, type, opts, cb) {
  ...
  if (type === 'forwarded-tcpip')
    ret = self._sshstream.forwardedTcpip(localChan, initWindow, maxPacket, opts);
  else if (type === 'x11')
    ret = self._sshstream.x11(localChan, initWindow, maxPacket, opts);
  else if (type === 'forwarded-streamlocal@openssh.com') {
    ret = self._sshstream.openssh_forwardedStreamLocal(localChan,
                                                       initWindow,
                                                       maxPacket,
                                                       opts);
  }
