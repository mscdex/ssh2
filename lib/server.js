...
function Server(cfg, listener) {
  ...
  this._srv = new net.Server(function(socket) {
    ...
    var sshstream = new SSH2Stream(streamcfg),
        client = new Client(streamcfg, sshstream, socket);
...
function Client(cfg, stream, socket) {
  EventEmitter.call(this);
  ...
  function onUSERAUTH_REQUEST(username, service, method, methodData) {
    ...
    if (method === 'keyboard-interactive') {
      ...
      ctx = new AuthContext(stream, username, service, method, methodData,
                            onAuthDecide);
    }
    ...
    if (authCtx) {
      if (!authCtx._initialResponse)
        ...
        authCtx._cleanup && authCtx.cleanup();
        authCtx.emit('abort');
      }
    }
  ...
  function onAuthDecide(ctx, allowed) {
    ...
    switch (info.type) {
      case 'session':
        accept = function() {
          ...
          stream.channelOpenConfirm(info.sender, localChan, Channel.MAX_WINDOW,
                                    Channel.PACKET_SIZE);
...
Client.prototype.forwardOut = function(bindAddr, bindPort, remoteAddr,
                                       remotePort, cb) {
  var opts = {
    bindAddr: bindAddr,
    bindPort: bindPort,
...
function Session(client, info, localChan) {
  ...
  function onREQUEST(info) {
    ...
    if (ending) {
      if (info.wantReply)
        client._sshstream.channelFailure(info.sender);
      return;
    }
    ...
    if (info.wantReply) {
      ...
      if (info.request !== 'shell'
        ...
        accept = function() {
          if (replied
              || ending
              || (channel && (!channel.writable || !channel.readable)))
            return;

          replied = true;

          return client._sshstream.channelSuccess(info.sender);
        };
      }
      ...
      reject = function() {
        ...
        return client._sshstream.channelFailure(info.sender);
      };
    }
    ...
    switch (info.request) {
      ...
      case 'shell':
        if (listenerCount(self, 'shell')) {
          accept = function() {
            ...
            if (info.wantReply)
              client._sshstream.channelSuccess(info.sender);

            chaninfo.type = self.type = info.request;

            return new Channel(chaninfo, client, { server: true });
          };
      ...
      case 'subsystem':
        accept = function() {
          ...
          chaninfo.type = self.type = (info.request + ':' + info.subsystem);

          var channel = new Channel(chaninfo, client, { server: true });
...
function AuthContext(stream, username, service, method, cb) {
  ...
  this._cbfinal = function(allowed) {
...
AuthContext.prototype.reject = function() {
  this._cleanup && this._cleanup();
  this._initialResponse = true;
  this._cbfinal(false);
};
...
function openChannel(self, type, opts, cb) {
  ...
  if (type === 'forwarded-tcpip') {
    ret = self._sshstream.forwardedTcpip(localChan, initWindow, maxPacket,
                                         opts.bindAddr, opts.bindPort,
                                         opts.remoteAddr, opts.remotePort);
  } else if (type === 'x11') {
    ret = self._sshstream.x11(localChan, initWindow, maxPacket, opts.originAddr,
                              opts.originPort);
  }
